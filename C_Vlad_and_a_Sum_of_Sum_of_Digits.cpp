...
vector<int> precompute_digit_sums(int n) {
    vector<int> cumulative_sums(n + 1, 0);
    for (int i = 1; i <= n; ++i) {
        cumulative_sums[i] = cumulative_sums[i - 1] + digit_sum(i);
    }
    return cumulative_sums;
}

int sum_of_digits_up_to_n(int n, const vector<int>& cumulative_sums) {
    return cumulative_sums[n];
}
...
int main() {
    ...
    vector<int> cumulative_sums = precompute_digit_sums(1000000);

    while (t--) {
        int n;
        cin >> n;

        int result = sum_of_digits_up_to_n(n, cumulative_sums);
