...
{
    ll length, breadth, a;
    cin >> length >> breadth >> a;
    ll ans = 0;
    ans += length / a;
    if (length % a != 0)
        ans++;
    ll  need = ans;
    ll  breadth_remaining = breadth - a;
    if (breadth_remaining <= 0)
        cout << ans;
    else if (breadth_remaining > 0 and breadth_remaining < a)
    {
        ans *= 2;
        cout << ans << endl;
    }
    else if (breadth_remaining >= a)
    {
        ans += (breadth_remaining / a) * need;
        if (breadth_remaining % a != 0)
        {
            ans += need;
        }
        cout << ans << endl;
